...
# Helper to show logo with fallback
def show_logo(width=300):
    lp = meta.get("logo_path", "logo.png")
    if lp and Path(lp).exists():
        st.image(lp, width=width, use_column_width=False)
    else:
        st.markdown(f"![Logo](https://via.placeholder.com/{width}x100?text=ANIM+Logo)")
...
if admin_ok:
    ...
    logo_file = st.sidebar.file_uploader("Upload logo image", type=["png","jpg","jpeg"], accept_multiple_files=False)
    if logo_file:
        Path("logo.png").write_bytes(logo_file.getbuffer())
